// This is a copy of the original file with small modifications.
// This is manly because we need a new version of the header file.
    ...
    DenseRTPSink *&resultSink)
{
  ...
  resultSink = (DenseRTPSink *)sink;
  return True;
}

Boolean DenseRTPSink::isRTPSink() const
{
  return True;
}
      ...
      fNumChannels(numChannels), fEstimatedBitrate(0)
{
  ...
  fTransmissionStatsDB = new DenseRTPTransmissionStatsDB(*this);
}
...
u_int32_t DenseRTPSink::convertToRTPTimestamp(struct timeval tv)
{
...
u_int32_t DenseRTPSink::presetNextTimestamp()
{
...
void DenseRTPSink::getTotalBitrate(unsigned &outNumBytes, double &outElapsedTime)
{
  struct timeval timeNow;
  gettimeofday(&timeNow, NULL);
...
void DenseRTPSink::resetPresentationTimes()
{
...
char const *DenseRTPSink::sdpMediaType() const
{
  return "data";
  // default SDP media (m=) type, unless redefined by subclasses
}

char *DenseRTPSink::rtpmapLine() const
{
  if (rtpPayloadType() >= 96)
  { // the payload format type is dynamic
    ...
    unsigned rtpmapFmtSize = strlen(rtpmapFmt) + 3 /* max char len */ + strlen(rtpPayloadFormatName()) + 20 /* max int len */ + strlen(encodingParamsPart);
...
char const *DenseRTPSink::auxSDPLine()
{
  return NULL; // by default
}

////////// DenseRTPTransmissionStatsDB //////////

DenseRTPTransmissionStatsDB::DenseRTPTransmissionStatsDB(DenseRTPSink &rtpSink)
    : fOurDenseRTPSink(rtpSink),
      fTable(HashTable::create(ONE_WORD_HASH_KEYS))
{
  fNumReceivers = 0;
}

DenseRTPTransmissionStatsDB::~DenseRTPTransmissionStatsDB()
{
  // First, remove and delete all stats records from the table:
  DenseRTPTransmissionStats *stats;
  while ((stats = (DenseRTPTransmissionStats *)fTable->RemoveNext()) != NULL)
  {
    delete stats;
  }
...
void DenseRTPTransmissionStatsDB ::noteIncomingRR(u_int32_t SSRC, struct sockaddr_in const &lastFromAddress,
                                                  unsigned lossStats, unsigned lastPacketNumReceived,
                                                  unsigned jitter, unsigned lastSRTime, unsigned diffSR_RRTime)
{
  DenseRTPTransmissionStats *stats = lookup(SSRC);
  if (stats == NULL)
  {
    // This is the first time we've heard of this SSRC.
    // Create a new record for it:
    stats = new DenseRTPTransmissionStats(fOurDenseRTPSink, SSRC);
    if (stats == NULL)
      return;
    add(SSRC, stats);
#ifdef DEBUG_RR
    fprintf(stderr, "Adding new entry for SSRC %x in DenseRTPTransmissionStatsDB\n", SSRC);
#endif
  }
...
void DenseRTPTransmissionStatsDB::removeRecord(u_int32_t SSRC)
{
  DenseRTPTransmissionStats *stats = lookup(SSRC);
  if (stats != NULL)
  {
...
DenseRTPTransmissionStatsDB::Iterator ::Iterator(DenseRTPTransmissionStatsDB &receptionStatsDB)
    : fIter(HashTable::Iterator::create(*(receptionStatsDB.fTable)))
{
}

DenseRTPTransmissionStatsDB::Iterator::~Iterator()
{
  delete fIter;
}

DenseRTPTransmissionStats *
DenseRTPTransmissionStatsDB::Iterator::next()
{
  char const *key; // dummy

  return (DenseRTPTransmissionStats *)(fIter->next(key));
}

DenseRTPTransmissionStats *DenseRTPTransmissionStatsDB::lookup(u_int32_t SSRC) const
{
  long SSRC_long = (long)SSRC;
  return (DenseRTPTransmissionStats *)(fTable->Lookup((char const *)SSRC_long));
}

void DenseRTPTransmissionStatsDB::add(u_int32_t SSRC, DenseRTPTransmissionStats *stats)
{
  long SSRC_long = (long)SSRC;
...
DenseRTPTransmissionStats::DenseRTPTransmissionStats(DenseRTPSink &rtpSink, u_int32_t SSRC)
    : fOurDenseRTPSink(rtpSink), fSSRC(SSRC), fLastPacketNumReceived(0),
...
DenseRTPTransmissionStats::~DenseRTPTransmissionStats() {}

void DenseRTPTransmissionStats ::noteIncomingRR(struct sockaddr_in const &lastFromAddress,
                                                unsigned lossStats, unsigned lastPacketNumReceived,
                                                ...
                                                unsigned diffSR_RRTime)
{
  ...
  // this receiver:
  u_int32_t newOctetCount = fOurDenseRTPSink.octetCount();
  ...
  u_int32_t newPacketCount = fOurDenseRTPSink.packetCount();
...
unsigned DenseRTPTransmissionStats::roundTripDelay() const
{
...
void DenseRTPTransmissionStats::getTotalOctetCount(u_int32_t &hi, u_int32_t &lo)
{
  hi = fTotalOctetCount_hi;
  lo = fTotalOctetCount_lo;
}

void DenseRTPTransmissionStats::getTotalPacketCount(u_int32_t &hi, u_int32_t &lo)
{
  hi = fTotalPacketCount_hi;
  lo = fTotalPacketCount_lo;
}

unsigned DenseRTPTransmissionStats::packetsReceivedSinceLastRR() const
{
...
int DenseRTPTransmissionStats::packetsLostBetweenRR() const
{
  if (!fAtLeastTwoRRsHaveBeenReceived)
    return 0;
